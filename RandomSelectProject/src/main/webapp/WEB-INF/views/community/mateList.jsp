	...
	function drawRestntList(){
		...
		$.ajax({
			...
			success : function(json){
				...
				var pageCount = Mate.ceil(totalCount / viewCount);
				var link = '';
				for(var idx = 1; idx <= pageCount; idx++){
					link += '<a id="'+idx+'" onclick ="restntListPage(this)" >['+idx+']</a>';
				}
				$('#mateListPage').append(link);
			}
		});
		
	}
	
	function drawMateDetail(obj){
			...
			<table border=1>
				...
				<c:forEach var="mate" items="${mates}">		
					<tr>
						...
						<input type="button"value="${mate.memName}" id="mateName" class="btn btn-info btn-sm" name="memName" onclick="drawMateDetail(this);"value="상세보기"class="btn btn-danger" 
